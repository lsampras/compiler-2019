...
set findFirst(enum nonterminal a){
    ...
    set c = 0,t=0;
    set firstset = 0;
    for(int i=0;i<RULESIZE;i++){ //traversing grammer for productions
        if(g[i].nt== a){
            temp = g[i].prod;
            t = getSet(EPS);
            while(temp!=NULL){//traversing a production
...
set findFollow(enum nonterminal a,enum nonterminal parent){
    ...
    set t = 0;;
    for(int i=0;i<RULESIZE;i++){//traversing grammar for productions
        temp = g[i].prod;
        t = 0UL;
        bool b=false;
        bool carry=false;

        while(temp!=NULL){//traversing single production
            ...
            if((!temp->node.is_leaf  && temp->node.data.nonterm == a || carry)){
                ...
                if(temp->next == NULL){
                    ...
                    if(temp->next->node.is_leaf && temp->next->node.data.term!= EPS){
...
struct node getnode(char * str){
    ...
    if(isupper(str[0]) == 0){
        ...
        if(symbolTable[hash] != NULL){
            ...
            while(strcmp(str,n->key) != 0){ //next if not equal
                n = n->next;
            }
        ...
        if(symbolTable[hash] != NULL){
            symbolnode * n = symbolTable[hash];
            while(strcmp(str,n->key) != 0){ //skip if not equal
                n = n->next;
            }
...
bool initTree(){
    ...
    while(1){
        ...
        if(n->tree->parent.is_leaf){//terminal matches
